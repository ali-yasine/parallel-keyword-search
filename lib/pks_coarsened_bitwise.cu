...
__global__ void expand_bitwise_coarsen_kernel(const CsrGraph* graph, const ull* frontier, ull* F_identifier, int* M, ull* C_indentifier, const int* min_activations, int bfs_level, float alpha, float avg_hops, const ull* keyword_nodes, int query_num) {
    
    int nodesStart = blockIdx.x * blockDim.x * coarsening_factor + threadIdx.x;
    int nodeEnd = (nodesStart + COARSENING_FACTOR > graph->num_nodes) ? graph->num_nodes : nodesStart + COARSENING_FACTOR;
    if (nodesStart < graph->num_nodes) {
        ull F_nodes = frontier[nodesStart / 64];
        ull C_nodes = C_indentifier[nodesStart / 64];
        ull fidentifier_nodes = F_identifier[nodesStart / 64];

        for(int node = nodesStart; node < nodeEnd; ++node) {

            ull node_bit = 1ULL << (node % 64); 
            if (F_nodes & node_bit) {
                if (! (C_nodes) & node_bit ) {
                    
                    int min_activation = min_activations[node];

                    if (min_activation > bfs_level) {
                        fidentifier_nodes |= node_bit;
                        continue;   
                    }
                    ...
                    for (int bfs_instance = 0; bfs_instance < query_num; ++bfs_instance) {
                        ...
                        for(int neighbor = graph->row_offsets[node]; neighbor < graph->row_offsets[node + 1]; ++neighbor) {
                            ...
                            if (!keyword_nodes[neighbor_id / 64] & (1ULL << (neighbor_id % 64))) {
...
void expand_bitwise_coarsen(const CsrGraph* graph_d, const ull* frontier_d, ull* F_identifier_d, int* M_d, ull* C_identifier_d, const int* min_activations_d, int bfs_level, float alpha, float avg_hops, const ull* keyword_nodes_d, int query_num, int num_nodes) {

    int num_threads = MAX_THREADS;     

    int numBlocks = (num_nodes + num_threads - 1) / (num_threads * coarsening_factor);

    expand_bitwise_coarsen_kernel<<<numBlocks, num_threads>>>(graph_d, frontier_d, F_identifier_d, M_d, C_identifier_d, min_activations_d, bfs_level, alpha, avg_hops, keyword_nodes_d, query_num, num_nodes);
